    ...
    private boolean handleStreamPayload(
        ...
        try {
            ...
            if (upstreamError != null) {
                failAndRefundIfPending(recordId, userId, recordSiteId, finalCost, upstreamError, "视频分析失败退款-" + finalModel);
            ...
            if (node.has("code") && node.get("code").canConvertToInt()) {
                ...
                if (code != 0 && code != 200) {
                    ...
                    failAndRefundIfPending(recordId, userId, recordSiteId, finalCost, msg, "视频分析失败退款-" + finalModel);
        ...
        try {
            ...
            okHttpClient.newCall(reqBuilder.build()).enqueue(new Callback() {
                ...
                public void onFailure(Call call, IOException e) {
                    failAndRefundIfPending(recordId, userId, recordSiteId, finalCost, e.getMessage(), "视频分析失败退款-" + finalModel);
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    try (ResponseBody responseBody = response.body()) {
                        if (!response.isSuccessful()) {
                            ...
                            failAndRefundIfPending(recordId, userId, recordSiteId, finalCost, "API Error: " + response.code(), "视频分析失败退款-" + finalModel);
                        ...
                        if (responseBody == null) {
                            failAndRefundIfPending(recordId, userId, recordSiteId, finalCost, "Empty response body", "视频分析失败退款-" + finalModel);
                            emitter.complete();
                            return;
                        }
                        ...
                        if (!isFinished && fullResponse.length() > 0) {
                            ...
                            failAndRefundIfPending(recordId, userId, recordSiteId, finalCost, "Stream ended without result", "视频分析失败退款-" + finalModel);
